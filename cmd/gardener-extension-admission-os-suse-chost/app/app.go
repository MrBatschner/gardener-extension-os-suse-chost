...
import (
	...
	admissioncmd "github.com/gardener/gardener-extension-os-suse-chost/pkg/admission/cmd"
...
const AdmissionName = "admission-os-suse-chost"

var log = logf.Log.WithName("gardener-extension-admission-os-suse-chost")

// NewAdmissionCommand creates a new command for running a suse chost admission webhook.
func NewAdmissionCommand(ctx context.Context) *cobra.Command {
	var (
		...
		mgrOpts  = &controllercmd.ManagerOptions{
			...
			WebhookCertDir:          "/tmp/admission-os-suse-chost-cert",
		}
	...
	cmd := &cobra.Command{
		Use: fmt.Sprintf("admission-%s", susechost.ControllerName),
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			memoryoneChostInstall.Install(mgr.GetScheme())
