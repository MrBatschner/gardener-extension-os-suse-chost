...
As these two fields are not sufficient to pass in any other of the several dozens of parameters that vSMP MemoryOne supports, it is possible to _inject_ additional key-value pairs through either `memoryTopology` or `systemMemory` by separating them vie a semicolon `;`:
...
It is possible to configure any key-value pair that configures the vSMP MemoryOne hypervisor by passing them in the `vsmpConfiguration` map. Note, that the keys must be the ones that vSMP MemoryOne understands.
...
Internally, this is a `map[string]string` hence, numbers/integers must be quoted.

**Please note** that semicola `;` are not allowed inside the values for `vsmpConfiguration` - if a semicolon is found in a value, it and anything that follows be stripped before it is further processed.
