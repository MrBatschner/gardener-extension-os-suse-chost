...
import (
	...
	memoryoneChostValidation "github.com/gardener/gardener-extension-os-suse-chost/pkg/apis/memoryonechost/validation"
...
func (s *shoot) validateShoot(_ context.Context, shoot *core.Shoot) error {
	...
	for i, worker := range shoot.Spec.Provider.Workers {
		...
		if errList := memoryoneChostValidation.ValidateOperatingSystemConfig(operatingSystemConfig, fldPath); len(errList) != 0 {
...
func isSupportedMachineImage(machineImageName string) bool {
	return machineImageName == memoryone.OSTypeMemoryOneCHost
}
